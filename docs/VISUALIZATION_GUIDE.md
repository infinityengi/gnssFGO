...
**Option A: Load Pre-configured Settings (Recommended)**
1. In Mapviz, click **File → Open Config**
2. Navigate to: `/workspace/fgo_ws/src/gnssFGO/online_fgo/launch/deutschland.mvc`
   - Full path: `/workspace/fgo_ws/src/gnssFGO/online_fgo/launch/deutschland.mvc`
   - You can paste this path directly in the file browser
3. Click **Open**
4. The visualization will auto-configure with:
   - Google Maps Hybrid background (satellite view)
   - Blue trajectory: `/novatel/gps/fix` (reference GNSS)
   - Red trajectory: `/deutschland/statePredictedNavFix` (predicted)
   - Green trajectory: `/deutschland/stateOptmizedNavFix` (optimized)
   - View centered on Aachen, Germany

**Note**: The "Wgs84Transformer not initialized" warnings are normal and will disappear once the config is loaded.
...
## Method 1.5: All-in-One Launch (Complete System)

Launch everything (preprocessor, FGO, LIO-SAM, RViz2, Mapviz, and bag player) in a single command.

### Prerequisites

Set the bag file path and start offset as environment variables:

```bash
export BAG_PATH=/workspace/fgo_ws/src/gnssFGO/bags/AC_0.db3
export START_OFFSET=60
```

### Launch Command

**Option 1: With exported environment variables**
```bash
cd /workspace/fgo_ws
source install/setup.bash
...
cd /workspace/fgo_ws
source install/setup.bash
BAG_PATH=/workspace/fgo_ws/src/gnssFGO/bags/AC_0.db3 START_OFFSET=60 ros2 launch online_fgo aachen_lc_all.launch.py
```

### What Gets Launched

This single command launches:
1. **GNSS Preprocessor** (in xterm window)
2. **Online FGO** (aachen_lc mode)
3. **LIO-SAM** nodes (image projection, feature extraction)
4. **RViz2** (3D visualization with point clouds)
5. **Mapviz** (2D GPS trajectory visualization)
6. **Bag Player** (in xterm window, plays from START_OFFSET)
7. **TF Publishers** (coordinate frame transforms)

### Configuration

- Load Mapviz config: `/workspace/fgo_ws/src/gnssFGO/online_fgo/launch/deutschland.mvc`
- RViz config: Auto-loaded from `LIOSAM/config/rviz2.rviz`
- Bag file: Specified by `BAG_PATH` environment variable
- Start offset: Specified by `START_OFFSET` (in seconds)

### Expected Windows

- **xterm 1**: GNSS Preprocessor logs
- **xterm 2**: Bag player controls
- **RViz2 window**: 3D visualization (LiDAR point clouds, odometry, path)
- **Mapviz window**: 2D map with GPS trajectories
- **Main terminal**: Online FGO and LIO-SAM logs
